...
几个规范需要关注
Execution API Execution Specs Engine API Consensus Specs EVM The Ethereum Virtual Machine — How does it work? - 深入浅出的介绍了 EVM 的工作原理 Event logs Understanding event logs on the Ethereum blockchain Understanding Logs: Deep Dive into eth_getLogs TODO
https://ethereum.org/en/developers/docs/evm/ Storage Layouts in EVM 深入理解 EVM 的存储布局非常重要，因为它直接影响到合约的 gas 消耗，以及合约的安全性。
What is smart contract storage layout ? Solidity 中有三种内存类型：storage, memory, calldata
memory 类型的变量和参数，用在函数的内部，只在函数执行期间存在，函数执行完毕后，内存被清空 calldata 类型是在外部调用函数时，传递的参数，只读，不能修改 storage 类型的变量，用来存储合约的状态，永久存在，直到合约被销毁 storage memory layout
...
如果一个类型不足 32 bytes，会被填充到 32 bytes；但是填充会浪费内存空间，但是带来了 gas 的节约，因为读写成本更低 但是在某些情况下，会把紧邻的不足 32 bytes 的变量包装在一起，可以节省内存空间，但是会增加 gas 消耗，因为读写成本更高，需要做更多的位运算 References 深入理解以太坊 - 简要介绍以太坊的架构 以太坊设计与实现 - 一本深入探讨以太坊实现的电子书 go-ethereum doc - geth 的官方文档 Ethereum Blog `,description:"",tags:null,title:"Deep Understanding of Ethereum",uri:"/blockchain/ethereum/index.html"},{content:`不需要面面俱到的记录和 Copy，只记录自己的一些疑惑，以及一些有意思的东西。
Golang 基础知识 Golang 的数据类型有哪些？引用类型和值类型的区别？什么时候用引用类型，什么时候用值类型？
Golang 文件系统操作 [译]Go文件操作大全 How to create a directory if it does not exist in Go Reference Everything about Golang - 之前记录的一些笔记 `,description:"",tags:["Golang"],title:"Golang",uri:"/programming-language/golang/index.html"},{content:`作为用户，Wallet 是区块链的入口；现阶段主流的 Wallet 类型有：HD Wallet，MultiSig wallet，Custodial wallet，Hardware wallet，Paper wallet，MPC Wallet 等。作为开发者，HD Wallet、MultiSig Wallet、MPC Wallet 是需要重点掌握的。
...
搞明白 hardhat 用 hardhat 和 openzeplin 写 erc20 合约，部署、运行等 开发工具、框架 工欲善其事，必先利其器。
开发工具 Remix - 一个基于浏览器的 IDE，可以用来编写、编译、部署智能合约。
Remix 内部也提供了一个交互式的学习 Solidity 的教程 Visiual Studio Code \u0026 Solidity Extension - 一个 VSCode 的插件，可以用来编写、编译、部署智能合约。
Jetbrains 系列 IDE \u0026 Solidity Extension - 一个 Jetbrains 系列 IDE 的插件，可以用来编写、编译、部署智能合约。
开发框架 Hardhat OpenZeplin Truffle ether.js Ganache TypeScript \u0026 JavaScript \u0026 ES6 \u0026 Node.js 学习资源 Solidity Documents
中文版 英文版 WTF 学院
WTF-Solidity 用 Go 来做以太坊开发 \u0026 Ethereum Development with Go
...
科学家社区
崔棉大师
语法 变量，数据类型和可见性 Solidity 有三种变量类型：
状态变量 本地变量 全局变量 在 Solidity 的全局命名空间中，存在一些特殊的变量，可以在任何地方访问，不需要声明；主要提供一些区块链信息和通用的处理函数。
区块和交易相关：block.basefee, msg.sender, msg.data (calldata), msg.value, msg.sig, tx.gasprice, tx.origin, block.gaslimit, block.number, block.timestamp, block.difficulty, block.coinbase, block.chainid, blockhash(uint blockNumber) returns (bytes32), gasleft() returns (uint256) abi 编解码函数 错误处理：assert, require, revert 数学和加密函数：keccak256, sha256, ripemd160, ecrecover, addmod, mulmod 等等 地址类型：\u003caddress\u003e.balance, \u003caddress\u003e.code 等等 合约相关：this, selfdestruct(address recipient) storage, memory, calldata Solidity 中有几种变量的存储类型
memory: 一些被临时使用的变量，比如函数参数、本地变量、在执行过程中动态创建的数组；一旦函数执行完毕，这些变量就会被销毁。 storage calldata What is the difference between Memory and Calldata in Solidity? When to use Storage vs. Memory vs. Calldata in Solidity 函数 (function) Payable function What are Payable Functions in Solidity? 合约安全 EIP ERC20 我们首先需要熟悉 ERC20 标准协议，可以看 ERC20 Token Standard，非常重要。ERC20 标准规范可以获得更好的重用性，无论是对 web3 应用还是交易所。
...
项目 tokenlon - 值得信赖的去中心化交易结算协议 `,description:"",tags:null,title:"Web3",uri:"/blockchain/web3/index.html"},{content:"",description:"",tags:null,title:"Java",uri:"/tags/java/index.html"},{content:`从 JDK8 开始，Java 做了非常大的改进，这反映出 Java 不是一个保守禁锢的语言，而是一个适应时代变化、进化的语言，去努力吸收其他语言的优秀特性。
TODO
https://pretius.com/blog/java-17-features/ https://www.evm.codes/playground?fork=shanghai https://mirror.xyz/xyyme.eth/dsU7KoQLyqiHrY0bQX2ETq1zkDYiW-3PtzxfzGwRdss New Features JDK8 ~ JDK20 的新特性 对开发人员有哪些影响 JDK8
Lambda 表达式 Stream API Optional 类 新的日期时间 API Nashorn JavaScript 引擎 Base64 并行数组 并发 API Type Annotations IO 新增 JVM 新增 JDK9
JShell 模块化系统 HTTP 2 客户端 改进的 Javadoc 改进的 Stream API Reactive Streams 改进的 CompletableFuture 改进的 Optional 改进的 @Deprecated 注解 改进的 try-with-resources 改进的 Diamond 操作符 改进的 Process API 改进的 JVM 改进的 GC 改进的 IO 改进的 CORBA 改进的 RMI 改进的 JMX 改进的 JAXP 改进的 JAXB 改进的 JAX-WS 改进的 JAX-RS 改进的 JMS 改进的 JDBC 改进的 JNDI JDK10
局部变量类型推断 基于时间的版本控制 并行 Full GC 改进的 G1 GC 改进的 Parallel GC 改进的 CMS GC 改进的 ZGC 改进的 Shenandoah GC 改进的 JVM 改进的 IO 改进的 JAXP 改进的 JAX-WS 改进的 JAX-RS 改进的 JMS 改进的 JDBC 改进的 JNDI JDK11
HTTP Client 改进的 GC 改进的 JVM 改进的 IO 改进的 JAXP 改进的 JAX-WS 改进的 JAX-RS 改进的 JMS 改进的 JDBC 改进的 JNDI `,description:"",tags:["Java"],title:"JDK 新特性",uri:"/programming-language/java/jdk-new-features/index.html"},{content:"",description:"",tags:null,title:"Tags",uri:"/tags/index.html"},{content:`测试网络 Goerli Testnet Goerli (Goerlitzer Testnet) Goerli Testnet Explorer Görli beacon Testnet 搭建私有以太坊私有网络 搭建私有网络
...
`,description:"",tags:null,title:"构建以太坊网络",uri:"/blockchain/ethereum/networks/index.html"},{content:"",description:"",tags:null,title:"Blockchain",uri:"/tags/blockchain/index.html"},{content:"",description:"",tags:null,title:"Web3",uri:"/tags/web3/index.html"},{content:`区块链的基础设施是非常重要的，这为开发者使用区块链、开发应用等提供了基础的支持，可以大大节省时间和资源。
工具篇 TODO
